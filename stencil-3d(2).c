...
{
    int i, j, k;
    svfloat32_t vec_A, vec_B, vec_C, vec_D, vec_E, vec_F, vec_G, vec_Result, vec_Seven;
        ...
        for (k = 1; k < nz - 1; k+=svcntw())
        {
            ...
            for (i = 1; i < nx - 1; i++)
            {
                for (j = 1; j < ny - 1; j++)
                {
                    ...
                    vec_A = svld1(pg, &A[i-1][j][0]);
                    vec_B = svld1(pg, &A[i][j-1][0]);
                    vec_C = svld1(pg, &A[i][j][k-1]);

                    // load A[i][j][k] vector
                    vec_D = svld1(pg, &A[i][j][0]);

                    // load A[i][j][k+1], A[i][j+1][k], and A[i+1][j][k] vectors
                    vec_E = svld1(pg, &A[i][j][k+1]);
                    vec_F = svld1(pg, &A[i][j+1][0]);
                    vec_G = svld1(pg, &A[i+1][j][0]);

                    // compute the sum of the vectors
                    vec_Result = svadd_f32_m(pg, vec_A, vec_B);
                    vec_Result = svadd_f32_m(pg, vec_Result, vec_C);
                    vec_Result = svadd_f32_m(pg, vec_Result, vec_D);
                    vec_Result = svadd_f32_m(pg, vec_Result, vec_E);
                    vec_Result = svadd_f32_m(pg, vec_Result, vec_F);
                    vec_Result = svadd_f32_m(pg, vec_Result, vec_G);
